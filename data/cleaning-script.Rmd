...
Now that both of our parent datasets are in formats that align with the needs of our shapefile data, we can begin the necessary preparations for our spatial join. Before performing this operation, we will first need to identify all inconsistencies in the `region` and `country` designations between our three data sources.

We will begin this process by investigating the differences between our university enrollment and dietary footprint data, first by examining the rows that exist in the former but not the latter and then by examining the rows that exist in the latter but not the former.
